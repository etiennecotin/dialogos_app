  ...
  for (p in inertiaVars) {
    if (!_reservedProps[p]) {
      ...
      if (_isObject(curProp)) {
        ...
        curDuration = Math.abs(curVelocity / _getNumOrDefault(curProp, "resistance", resistance));
        curVal = parseFloat(getVal(target, p));
...
export var InertiaPlugin = {
  ...
  init: function init(target, vars, tween, index, targets) {
    ...
    var cache = target._gsap,
        getVal = cache.get,
        dur = vars.duration,
        preventOvershoot = vars.preventOvershoot,
        resistance = _getNumOrDefault(vars, "resistance", _config.resistance),
        duration = _isNumber(dur) ? dur : _calculateTweenDuration(target, vars, _isObject(dur) && dur.max || 10, _isObject(dur) && dur.min || 0.2, _isObject(dur) && "overshoot" in dur ? +dur.overshoot : preventOvershoot ? 0 : 1),
        p,
        curProp,
        curVal,
        unit,
